...
Use this workflow only when requested by the user or required by the active agent instructions. The coordinator owns scope, triage, decisions, budget, integration, and the final report. Subagents provide bounded evidence or implement accepted work.

Target-repository instructions remain authoritative for repository-specific policy, architecture, coding standards, contribution rules, and acceptance criteria.

## 1. Establish scope and state

1. Determine the base and head revisions, the requested behavior, acceptance criteria, and whether uncommitted changes are in scope. Prefer the target repository's documented base; otherwise determine the likely merge base and state the assumption.
...
3. Create `.agents/pr-reviews/state/<branch-or-pr>.md` in the target working tree from `.agents/pr-reviews/STATE_TEMPLATE.md`, or resume the matching state file. State files are local working memory and must remain untracked unless the user explicitly chooses to preserve one elsewhere. If the target repository does not already ignore this path, prefer a local Git exclude such as `.git/info/exclude` rather than modifying the repository solely for review state.
