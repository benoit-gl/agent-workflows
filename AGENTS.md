...
- Let that workflow discover repository-specific policy from ordinary human-facing contribution, ownership, security, and path-scoped documentation, including conventional host locations. Do not require a target repository to contain agent-specific instruction files.
- Record a delivery mode before work begins. Review, local editing, local commits, and pushing to an existing PR are distinct authorities; never infer a stronger authority from a weaker request. This workflow does not merge PRs.
...
- When a commit is created after review, verify its committed content matches the reviewed state. For PR delivery, re-resolve the current base and head and verify required remote checks on the authoritative check target rather than assuming they attach to the head SHA.
