...
The repository CI `verify` job is authoritative for mechanical verification of
a pull request. Contributors can run `npm run check` locally before pushing, but
the pull-request description does not need a verification section or copied CI
results. A pull request is complete only when its required CI succeeds for the
relevant final revision.

If required CI cannot run because of an external limitation, keep the work
explicitly incomplete until that verification is available.
...
Squash merge is the only permitted merge method. Write the pull-request title
and description as the final squash commit title and message. The description
must state the resulting change, not temporary branch or review-process status,
and it does not need a separate verification section. Update it during review
when the final change makes the existing description inaccurate or incomplete.
...
- confirm that all affected workflow and documentation artifacts agree; and
- ensure that the pull-request title and description accurately state the final
  change.

CI performs the required mechanical verification on the pull request. Do not
duplicate passing CI results in the pull-request description.

## Verification tooling

The local verification entry point used by CI is:
